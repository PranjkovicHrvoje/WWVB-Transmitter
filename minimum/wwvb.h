...
private:
	...
	//Bresenham’s Algorithm for freq not equal to 60 kHz
	volatile uint8_t nominator   = 0;
	volatile uint8_t denominator   = 0;
	volatile uint8_t accumulated_fractional_ticks = 0;
	volatile uint8_t ticks_per_period = 0;
...
#else
#if (F_CPU == 16000000)
		set(12030, 30075, 48120, 60150); //for ICR1 134 set(5970,11940,29851,47761,59701)
...
//OC2B :  3 PD3
		...
		// 16MHz / (2*60kHz) = 133 + 1/3
		ticks_per_period = 133;
		nominator = 1;
		denominator = 3;
		ICR1 = ticks_per_period; // Set PWM to 60kHz (16MHz / (2*133)) = 60150Hz
		// Yes, its 133 and not 132 because the compare is double sided : 0->133->132->1
		PWM_HIGH = 66; // ~50% duty cycle
		PWM_LOW = 6; // ~5% duty cycle
#elif (F_CPU == 8000000)
		ticks_per_period = 67;
		ICR1 = ticks_per_period; // Set PWM to 60.6kHz (8MHz / (2*66)) = 60606Hz
	...
	void interrupt_routine()
	{
		...
		if (isr_count >= (WWVB_ENDOFBIT + WWVB_EOB_CAL[_is_odd_bit]))
		{
			...
			if (accumulated_fractional_ticks < denominator) {
				ICR1 = ticks_per_period;
				set(12030, 30075, 48120, 60150);
			} else {
				ICR1 = ticks_per_period+1;
				accumulated_fractional_ticks -= denominator;
				set(11940,29851,47761,59701);
			}
	...
	void set_time(uint8_t &_hour, uint8_t &_mins,
		uint8_t &_DD, uint8_t &_MM, uint8_t &_YY,
		uint8_t _daylight_savings = 0)
	{
